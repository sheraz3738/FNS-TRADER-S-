...
class _FnsAppState extends State<FnsApp> {
  ...
  Future<pw.Document> _buildBillPdf(OrderData order) async {
    final doc = pw.Document();
    final logoData = await DefaultAssetBundle.of(context).load('fns_logo.png');
    final logo = pw.MemoryImage(logoData.buffer.asUint8List());
    final lines = order.items.split('\n').where((e) => e.trim().isNotEmpty).toList();
    ...
    doc.addPage(
      pw.Page(
        ...
        build: (_) => pw.Column(
          ...
          children: [
            pw.Center(child: pw.Image(logo, height: 90, fit: pw.BoxFit.contain)),
            pw.SizedBox(height: 8),
            pw.Center(child: pw.Text("BA BA FALAK NAZ & SON'S TRADER'S", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold))),
            pw.SizedBox(height: 10),
            pw.Divider(),
            pw.Text('Order No: ${order.orderNo}'),
            pw.Text('Date: ${order.dateTime}'),
            pw.SizedBox(height: 8),
            pw.Text('Customer: ${order.customer}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            pw.Text('Phone: ${order.phone}'),
            pw.Text('Address: ${order.address}'),
            pw.SizedBox(height: 12),
            pw.Text('Items', style: pw.TextStyle(fontSize: 15, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 6),
            ...lines.map((line) => pw.Padding(padding: const pw.EdgeInsets.only(bottom: 4), child: pw.Text(line))),
            pw.Divider(),
            pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text('Grand Total: Rs. ${order.total}', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold))),
            pw.Text('Payment: ${order.payment}'),
            pw.SizedBox(height: 20),
            pw.Center(child: pw.Text('Thank you for shopping with FNS TRADER\'S', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
          ],
        ),
      ),
    );
  ...
  Future<pw.Document> _buildThermalBillPdf(OrderData order) async {
    final doc = pw.Document();
    final logoData = await DefaultAssetBundle.of(context).load('fns_logo.png');
    final logo = pw.MemoryImage(logoData.buffer.asUint8List());
    final lines = order.items.split('\n').where((e) => e.trim().isNotEmpty).toList();
    doc.addPage(
      pw.Page(
        ...
        build: (_) => pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.stretch,
          children: [
            pw.Center(child: pw.Image(logo, height: 55, fit: pw.BoxFit.contain)),
            pw.SizedBox(height: 4),
            pw.Center(child: pw.Text("BA BA FALAK NAZ & SON'S TRADER'S", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
            pw.SizedBox(height: 6),
            pw.Text('Order: ${order.orderNo}'),
            pw.Text('Date: ${order.dateTime}'),
            pw.Text('Customer: ${order.customer}'),
            pw.Text('Phone: ${order.phone}'),
            pw.Text('Address: ${order.address}'),
            pw.SizedBox(height: 6),
            ...lines.map((line) => pw.Padding(padding: const pw.EdgeInsets.only(bottom: 2), child: pw.Text(line, style: const pw.TextStyle(fontSize: 8)))),
            pw.Divider(),
            pw.Text('TOTAL: Rs. ${order.total}', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
            pw.Text('Payment: ${order.payment}', style: const pw.TextStyle(fontSize: 8)),
            pw.SizedBox(height: 8),
  ...
  Future<void> _printA4(OrderData order) async {
    final doc = await _buildBillPdf(order);
    await Printing.layoutPdf(onLayout: (_) async => doc.save());
  }

  Future<void> _printThermal(OrderData order) async {
    final doc = await _buildThermalBillPdf(order);
    await Printing.layoutPdf(onLayout: (_) async => doc.save());
  }

  Future<void> _shareBill(OrderData order) async {
    final doc = await _buildBillPdf(order);
    await Printing.sharePdf(bytes: await doc.save(), filename: '${order.orderNo}.pdf');
  }

  void _showBillActions(OrderData order) {
    showModalBottomSheet(
      context: context,
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Bill Options', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              ListTile(leading: const Icon(Icons.receipt_long), title: const Text('Print Thermal 80mm'), onTap: () { Navigator.pop(sheetContext); _printThermal(order); }),
              ListTile(leading: const Icon(Icons.print), title: const Text('Print A4'), onTap: () { Navigator.pop(sheetContext); _printA4(order); }),
              ListTile(leading: const Icon(Icons.share), title: const Text('Send Bill to Customer'), subtitle: const Text('Share PDF via WhatsApp or another app'), onTap: () { Navigator.pop(sheetContext); _shareBill(order); }),
            ],
          ),
        ),
      ),
  ...
  Future<void> placeOrder({
    ...
    final orderNo = 'FNS-${now.millisecondsSinceEpoch}';
    ...
    final itemText = cart.entries
        .map((entry) => '${entry.key.name} x ${entry.value} @ Rs. ${entry.key.price} = Rs. ${entry.key.price * entry.value}')
        .join('\n');
                                  ...
                                  itemBuilder: (_, index) {
                                    ...
                                    return Card(
                                      child: ExpansionTile(
                                        ...
                                        children: [
                                          Align(alignment: Alignment.centerLeft, child: Text('Order No: ${order.orderNo}')),
                                          const SizedBox(height: 4),
                                          Align(alignment: Alignment.centerLeft, child: Text('Date: ${order.dateTime}')),
                                          const SizedBox(height: 6),
                                          ...
                                          SizedBox(
                                            ...
                                            child: OutlinedButton.icon(
                                              onPressed: () => _showBillActions(order),
                                              icon: const Icon(Icons.receipt_long),
                                              label: const Text('Print / Send Bill'),
                                            ),
                                          ),
                                        ],
                                      ),
