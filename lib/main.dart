...
class _FnsAppState extends State<FnsApp> {
  ...
  Future<void> printInvoice(OrderData order, {bool isThermal = false}) async {
    ...
    pdf.addPage(pw.Page(pageFormat: pageFormat, build: (c) {
      return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
        ...
        pw.Container(width: double.infinity, padding: const pw.EdgeInsets.all(6), color: PdfColors.grey300, child: pw.Center(child: pw.Text('Welcome to FNS Traders', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)))),
  ...
  void startHeadlineScroll() async {
    ...
    while (mounted) {
      ...
      if (!mounted) return; await Future.delayed(const Duration(milliseconds: 500)); headlineController.jumpTo(0);
    }
  }
  ...
  Widget getCurrentPage(){
    switch(bottomIndex){
      case 0: return homePage(); case 1: return cartFullPage(); case 2: return favFullPage(); case 3: return aboutFullPage(); case 4: return adminLoginPage(); default: return homePage();
    }
  }
  ...
  Widget homePage() {
    ...
    return RefreshIndicator(onRefresh: loadSavedData, child: ListView(padding: const EdgeInsets.fromLTRB(14,10,14,100), children: [
      ...
      if (visible.isEmpty) const Padding(padding: EdgeInsets.all(30), child: Center(child: Text('No products found'))) else Column(children: visible.map((p)=> Card(margin: const EdgeInsets.only(bottom:10), child: ListTile(contentPadding: const EdgeInsets.all(10), leading: productImage(p,size:58), title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${p.category}\nRs. ${p.price}'), isThreeLine:true, trailing: Column(children:[IconButton(padding: EdgeInsets.zero, onPressed: ()=>toggleFavorite(p), icon: Icon(favorites.contains(p.name)? Icons.favorite: Icons.favorite_border)), IconButton(padding: EdgeInsets.zero, onPressed: p.inStock? ()=>addToCart(p):null, icon: const Icon(Icons.add_shopping_cart))])))).toList()),
    ]));
  }
...
class _AdminPanelFullState extends State<AdminPanelFull> {
  ...
  @override Widget build(BuildContext context){
    return DefaultTabController(length: 2, child: Scaffold(appBar: AppBar(title: const Text('Admin Panel'), backgroundColor: Colors.green, bottom: const TabBar(tabs: [Tab(text: 'Stock'), Tab(text: 'Orders')])), body: TabBarView(children: [
      SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
        ...
        TextField(controller: _name, decoration: const InputDecoration(labelText: 'Product Name', border: OutlineInputBorder()), onChanged: (v){ setState(()=> _cat.text = widget.detectCategory(v)); }), const SizedBox(height:8), 
        ...
        TextField(controller: _cat, decoration: const InputDecoration(labelText: 'Category (Auto Detect)', border: OutlineInputBorder())), const SizedBox(height:8), 
        Row(children:[ElevatedButton(onPressed: pickImage, child: const Text('Pick Photo')), const SizedBox(width:10), if(_img.isNotEmpty) Image.memory(base64Decode(_img), width:60, height:60)]), const SizedBox(height:8), 
        ...
        const SizedBox(height:8), Text('Current Limit: Rs. ${widget.minLimit}'),
        ...
        const SizedBox(height:8), Wrap(spacing:6, children:[0,500,1000,1500,2000,5000,10000,20000].map((v)=> ChoiceChip(label: Text(v==0 ? 'No Limit' : 'Rs. $v'), selected: widget.minLimit==v, onSelected: (_){ widget.onLimitChange(v); _limitCustom.text = v.toString(); setState((){}); })).toList()),
        const Divider(height:30), 
        const Text('Stock ON/OFF & Delete', style: TextStyle(fontWeight: FontWeight.bold)),
        Column(children: widget.products.map((p)=> Card(child: ListTile(leading: p.imageBase64.isNotEmpty? Image.memory(base64Decode(p.imageBase64), width:40, height:40, errorBuilder: (a,b,c)=>const Icon(Icons.image)): const Icon(Icons.medication), title: Text(p.name), subtitle: Text('Rs. ${p.price} - ${p.category}'), trailing: Row(mainAxisSize: MainAxisSize.min, children:[Switch(value: p.inStock, onChanged: (v){ setState(()=>p.inStock=v); widget.onUpdate(widget.products); }), IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: (){ final list=List<Product>.from(widget.products)..remove(p); widget.onUpdate(list); })])))).toList()),
      ])),
