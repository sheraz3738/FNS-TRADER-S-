...
class _FnsAppState extends State<FnsApp> {
  ...
  Future<void> printInvoice(OrderData order, {bool isThermal = false}) async {
    ...
    pdf.addPage(pw.Page(pageFormat: pageFormat, build: (c) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
      ...
      pw.Container(width: double.infinity, padding: const pw.EdgeInsets.all(6), color: PdfColors.grey300, child: pw.Center(child: pw.Text('Welcome to FNS Traders - Home Delivery Available', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)))),
      pw.Divider(thickness: 1), pw.SizedBox(height: 5),
      pw.Align(alignment: pw.Alignment.centerLeft, child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('Order No: ${order.orderNo}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)), pw.Text('Date: ${order.dateTime}'), pw.Text('Customer: ${order.customer}'), pw.Text('Phone: ${order.phone}'), pw.Text('Address: ${order.address}'), pw.Divider(),
        pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('ITEM', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)), pw.Text('QTY', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)), pw.Text('PRICE', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))]),
        pw.Divider(), pw.Text(order.items), pw.Divider(),
        pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('TOTAL:', style: pw.TextStyle(fontSize: isThermal? 14 : 16, fontWeight: pw.FontWeight.bold)), pw.Text('Rs. ${order.total}', style: pw.TextStyle(fontSize: isThermal? 14 : 16, fontWeight: pw.FontWeight.bold))]),
        pw.SizedBox(height: 5), pw.Text('Payment: ${order.payment}'), pw.SizedBox(height: 10), pw.Center(child: pw.Text('Thank You!', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
      ])),
    ]))));
    await Printing.layoutPdf(onLayout: (f) async => pdf.save());
  }

  @override void initState() { super.initState(); _headlineController = ScrollController(); loadSavedData(); WidgetsBinding.instance.addPostFrameCallback((_) => _startHeadlineScroll()); }
  void _startHeadlineScroll() async { await Future.delayed(const Duration(milliseconds: 500)); while (mounted) { if (!_headlineController.hasClients) { await Future.delayed(const Duration(milliseconds: 500)); continue; } final max = _headlineController.position.maxScrollExtent; if (max <= 0) { await Future.delayed(const Duration(seconds: 1)); continue; } await _headlineController.animateTo(max, duration: const Duration(seconds: 25), curve: Curves.linear); if (!mounted) return; await Future.delayed(const Duration(milliseconds: 500)); _headlineController.jumpTo(0); } }
  @override void dispose() { _headlineController.dispose(); super.dispose(); }
  Future<void> loadSavedData() async { final prefs = await SharedPreferences.getInstance(); final sp = prefs.getString('products'); final sf = prefs.getStringList('favorites'); final so = prefs.getString('orders'); minOrderLimit = prefs.getInt('minOrderLimit')??0; if (sp!=null && sp.isNotEmpty) { try { products = (jsonDecode(sp) as List).map((e) => Product.fromJson(Map<String,dynamic>.from(e))).toList(); } catch(_){} } if (sf!=null) { favorites..clear()..addAll(sf); } if (so!=null && so.isNotEmpty) { try { orders..clear()..addAll((jsonDecode(so) as List).map((e) => OrderData.fromJson(Map<String,dynamic>.from(e)))); } catch(_){} } if (mounted) setState(()=>loading=false); }
  ...
  int get cartCount { int c=0; for(final q in cart.values) c+=q; return c; } int get cartTotal { int t=0; for(final e in cart.entries) t+=e.key.price*e.value; return t; }
  List<Product> get filteredProducts { final q=searchText.toLowerCase().trim(); return products.where((p){ final cat=selectedCategory=='All'||p.category==selectedCategory; final s=q.isEmpty||p.name.toLowerCase().contains(q); return cat&&s; }).toList(); }
  Widget productImage(Product p,{double size=70}){ if(p.imageBase64.isNotEmpty){ try{ return ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode(p.imageBase64), width:size, height:size, fit:BoxFit.cover)); }catch(_){} } return Container(width:size,height:size,decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: Colors.grey.shade200), child: const Icon(Icons.shopping_bag,size:35)); }
  ...
  void increaseQuantity(Product p)=>setState(()=>cart[p]=(cart[p]??0)+1); void decreaseQuantity(Product p){ setState((){ final q=cart[p]??0; if(q<=1) cart.remove(p); else cart[p]=q-1; }); }
  ...
  @override Widget build(BuildContext context) {
    return MaterialApp(debugShowCheckedModeBanner: false, home: Scaffold(
      body: Stack(children: [
          [homePage(), cartFullPage(), favFullPage(), aboutFullPage(), adminLoginPage()][bottomIndex],
          Positioned(left: _fabPosition.dx, top: _fabPosition.dy, child: GestureDetector(onPanUpdate: (d) { setState(()=> _fabPosition = Offset(_fabPosition.dx + d.delta.dx, _fabPosition.dy + d.delta.dy)); }, child: FloatingActionButton(backgroundColor: const Color(0xFF25D366), onPressed: ()=>_openWhatsApp(), child: const Icon(Icons.whatsapp, color: Colors.white, size: 32)))),
        ]),
  ...
  Widget homePage() {
    ...
    return RefreshIndicator(onRefresh: loadSavedData, child: ListView(padding: const EdgeInsets.fromLTRB(14,10,14,100), children: [
      Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 10), decoration: BoxDecoration(color: Colors.blue.shade900, borderRadius: BorderRadius.circular(10)),
        child: SingleChildScrollView(controller: _headlineController, scrollDirection: Axis.horizontal, child: const Row(children: [SizedBox(width: 10), Text('بابا فلک ناز رحمتہ اللہ علیہ اینڈ سنز ٹریڈرز | BABA FALAK NAZ & SONS TRADERS | WELCOME TO FNS TRADERS | خوش آمدید | ', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w700)), SizedBox(width: 100)]))),
      ...
      const SizedBox(height: 12),
      SizedBox(height: 48, child: ListView(scrollDirection: Axis.horizontal, children: [_catChip('All'),...categories.map(_catChip)])),
      const SizedBox(height: 8), Text('${visible.length} product(s)', style: const TextStyle(fontWeight: FontWeight.w600)), const SizedBox(height: 6),
      if (visible.isEmpty) const Padding(padding: EdgeInsets.all(30), child: Center(child: Text('No products found'))) else...visible.map((p)=> Card(margin: const EdgeInsets.only(bottom:10), child: ListTile(contentPadding: const EdgeInsets.all(10), leading: productImage(p,size:58), title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${p.category}\nRs. ${p.price}'), isThreeLine:true, trailing: Column(children:[IconButton(padding: EdgeInsets.zero, onPressed: ()=>toggleFavorite(p), icon: Icon(favorites.contains(p.name)? Icons.favorite: Icons.favorite_border)), IconButton(padding: EdgeInsets.zero, onPressed: p.inStock? ()=>addToCart(p):null, icon: const Icon(Icons.add_shopping_cart))])))),
    ]));
  }

  Widget _catChip(String c)=> Padding(padding: const EdgeInsets.only(right:8), child: ChoiceChip(label: Text(c), selected: selectedCategory==c, onSelected: (_)=>setState(()=>selectedCategory=c)));
  Widget cartFullPage()=> Scaffold(appBar: AppBar(title: Text('Cart (${cartCount}) - Rs. $cartTotal'), backgroundColor: Colors.green), body: Column(children:[Expanded(child: cart.isEmpty? const Center(child: Text('Your cart is empty')): ListView(padding: const EdgeInsets.all(12), children: cart.entries.map((e)=> Card(child: ListTile(leading: productImage(e.key,size:55), title: Text(e.key.name), subtitle: Text('Rs. ${e.key.price} x ${e.value}'), trailing: Row(mainAxisSize: MainAxisSize.min, children:[IconButton(onPressed: ()=>decreaseQuantity(e.key), icon: const Icon(Icons.remove_circle)), Text('${e.value}', style: const TextStyle(fontWeight: FontWeight.bold)), IconButton(onPressed: ()=>increaseQuantity(e.key), icon: const Icon(Icons.add_circle))])))).toList())), if(cart.isNotEmpty) Padding(padding: const EdgeInsets.all(12), child: Column(children:[Text('Total: Rs. $cartTotal', style: const TextStyle(fontSize:20, fontWeight: FontWeight.bold)), const SizedBox(height:10), SizedBox(width: double.infinity, child: ElevatedButton(onPressed: ()=>showCheckoutSheet(), child: const Text('Checkout')))]))]));
...
class _AdminPanelFullState extends State<AdminPanelFull> {
  final _name=TextEditingController(); final _price=TextEditingController(); final _cat=TextEditingController(); String _img=''; final picker=ImagePicker();
  ...
  void _showPrintOptions(OrderData order){ showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Print Bill'), content: const Text('کون سا پرنٹ نکالنا ہے؟'), actions: [TextButton(onPressed: (){ Navigator.pop(context); widget.onPrint(order, isThermal: false); }, child: const Text('A4 Size')), TextButton(onPressed: (){ Navigator.pop(context); widget.onPrint(order, isThermal: true); }, child: const Text('Thermal 80mm'))])); }
  @override Widget build(BuildContext context)=> DefaultTabController(length: 2, child: Scaffold(appBar: AppBar(title: const Text('Admin Panel'), backgroundColor: Colors.green, bottom: const TabBar(tabs: [Tab(text: 'Stock'), Tab(text: 'Orders')])), body: TabBarView(children: [
    SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[const Text('Add Stock with Picture', style: TextStyle(fontWeight: FontWeight.bold, fontSize:18)), TextField(controller: _name, decoration: const InputDecoration(labelText: 'Product Name', border: OutlineInputBorder()), onChanged: (v){ setState(()=> _cat.text = widget.detectCategory(v)); }), const SizedBox(height:8), TextField(controller: _price, decoration: const InputDecoration(labelText: 'Price', border: OutlineInputBorder()), keyboardType: TextInputType.number), const SizedBox(height:8), TextField(controller: _cat, decoration: const InputDecoration(labelText: 'Category (Auto Detect)', border: OutlineInputBorder())), const SizedBox(height:8), Row(children:[ElevatedButton(onPressed: pickImage, child: const Text('Pick Photo')), const SizedBox(width:10), if(_img.isNotEmpty) Image.memory(base64Decode(_img), width:60, height:60)]), const SizedBox(height:8), SizedBox(width: double.infinity, child: ElevatedButton(onPressed: (){ if(_name.text.isEmpty) return; final finalCat = _cat.text.isEmpty? widget.detectCategory(_name.text) : _cat.text; final p=Product(name: _name.text, price: int.tryParse(_price.text)??0, category: finalCat, imageBase64: _img); final list=List<Product>.from(widget.products)..add(p); widget.onUpdate(list); _name.clear(); _price.clear(); _cat.clear(); setState(()=>_img=''); }, child: const Text('Add Stock'))), const Divider(height:30), Text('Limit Fix - Current: Rs. ${widget.minLimit}'), Wrap(spacing:6, children:[0,500,1000,1500,2000,2500,5000].map((v)=> ChoiceChip(label: Text('Rs. $v'), selected: widget.minLimit==v, onSelected: (_)=>widget.onLimitChange(v))).toList()), const Divider(height:30), const Text('Stock ON/OFF & Delete', style: TextStyle(fontWeight: FontWeight.bold)),...widget.products.map((p)=> Card(child: ListTile(leading: p.imageBase64.isNotEmpty? Image.memory(base64Decode(p.imageBase64), width:40, height:40, errorBuilder: (a,b,c)=>const Icon(Icons.image)): const Icon(Icons.medication), title: Text(p.name), subtitle: Text('Rs. ${p.price} - ${p.category} - ${p.inStock? "In Stock": "Out of Stock"}'), trailing: Row(mainAxisSize: MainAxisSize.min, children:[Switch(value: p.inStock, onChanged: (v){ setState(()=>p.inStock=v); widget.onUpdate(widget.products); }), IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: (){ final list=List<Product>.from(widget.products)..remove(p); widget.onUpdate(list); })])))),])),
    widget.orders.isEmpty? const Center(child: Text('کوئی آرڈر نہیں')) : ListView.builder(itemCount: widget.orders.length, itemBuilder: (c,i){ final o=widget.orders[widget.orders.length-1-i]; return Card(margin: const EdgeInsets.all(8), child: ListTile(title: Text(o.orderNo, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${o.customer} - ${o.phone}\n${o.items}\nRs. ${o.total} - ${o.dateTime}'), isThreeLine: true, trailing: IconButton(icon: const Icon(Icons.print, color: Colors.green), onPressed: ()=> _showPrintOptions(o)))); }),
  ]))));
}
